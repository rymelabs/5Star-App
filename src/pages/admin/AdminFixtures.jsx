...
const AdminFixtures = () => {
  ...
  const filteredFixtures = React.useMemo(() => {
    ...
    return fixtures.filter(fixture => {
      const homeTeam = teams.find(t => t.id === fixture.homeTeamId)?.name || '';
      const awayTeam = teams.find(t => t.id === fixture.awayTeamId)?.name || '';
      const comp = fixture.competition || '';
      const status = fixture.status || '';

      return homeTeam.toLowerCase().includes(q) ||
        awayTeam.toLowerCase().includes(q) ||
        comp.toLowerCase().includes(q) ||
        status.toLowerCase().includes(q);
    });
  }, [fixtures, searchQuery, teams]);
      ...
      <div className="space-y-6">
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <Search className="w-5 h-5 text-gray-500 group-focus-within:text-brand-purple transition-colors" />
          </div>
          <input
            type="text"
            placeholder="Search by team, competition or status..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-12 pr-12 py-3.5 bg-dark-800/50 border border-white/5 rounded-2xl text-white placeholder:text-gray-500 focus:outline-none focus:border-brand-purple/50 focus:bg-dark-800 transition-all shadow-lg"
          />
          {searchQuery && (
            <button
        ...
        {fixtures.length > 0 ? (
          <div className="space-y-6">
            ...
            {filteredFixtures.length === 0 && searchQuery && (
              <div className="card text-center py-12 bg-[#0b1020]/30 border-white/5">
                ...
                <p className="text-sm text-white/40">No matches found for "{searchQuery}". Try a different term.</p>
                <button
                  onClick={() => setSearchQuery('')}
                  className="mt-4 text-brand-purple text-xs font-bold uppercase tracking-widest hover:underline"
                >
                  Clear search
                </button>
              </div>
